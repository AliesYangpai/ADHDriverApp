package com.adhdriver.work.ui.widget.circle;
...
//            Color.argb(MIN_ALPHA, RED, GREEN, BLUE),
    ...
    private int innerCircleStrokeWidth = 100;
    ...
    private Paint mPaintPen = new Paint();



    public WaitOrderProgress(Context context) {
        super(context);
        init();
    }

    public WaitOrderProgress(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public WaitOrderProgress(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void drawCirlce(Canvas canvas) {
        ...
        // 设置颜色为红色
        mPaint.setColor(Color.parseColor("#3d4d5e"));
        /**
         ...
         */
        mPaint.setStrokeWidth(innerCircleStrokeWidth);

        canvas.drawCircle(getMeasuredWidth() / 2, getMeasuredHeight() / 2, getRadius(), mPaint); //画第一个圆（初始半径较大的）

    }


    private void drwaText(Canvas canvas,String text) {


        mPaintPen.setStrokeWidth(3);
        mPaintPen.setTextSize(30);
        mPaintPen.setColor(Color.WHITE);
        mPaintPen.setAntiAlias(true);
        mPaintPen.setTextAlign(Paint.Align.LEFT);
        mPaintPen.setStyle(Paint.Style.FILL);

        Rect bounds = new Rect();
        mPaintPen.getTextBounds(text, 0, text.length(), bounds);
        Paint.FontMetricsInt fontMetrics = mPaintPen.getFontMetricsInt();
        int baseline = (getMeasuredHeight() - fontMetrics.bottom + fontMetrics.top) / 2 - fontMetrics.top;
        canvas.drawText(text,getMeasuredWidth() / 2 - bounds.width() / 2, baseline, mPaintPen);
    }
...
//        float target = raduis * (doughnutRaduisPercent-doughnutWidthPercent-doughnutWidthPercent/2) +WAVE_WIDTH;


        float target = raduis * doughnutRaduisPercent + raduis * doughnutWidthPercent / 2 - innerCircleStrokeWidth/2 ;
